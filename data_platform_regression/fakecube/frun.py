...
def frun(fresult_id,g_id,url):
	re_file="%s/mysite/result/%s.txt" % (BASE_DIR,g_id)
	stand_file="%s/mysite/stand/%s.txt" % (BASE_DIR,g_id)
	cmd="curl \"%s\" > %s" % (url,re_file) 
	print cmd
	os.system(cmd)

	dif_ret=compFile(stand_file,re_file)
	if (dif_ret!=0):
		#diff_file="%s/static/fakecubediff/%s_%s.txt" % (BASE_DIR,fresult_id,g_id)
		#cmd="diff %s %s > %s" % (stand_file,re_file,diff_file)
		diff_file="%s/static/fakecubediff/%s_%s.html" % (BASE_DIR,fresult_id,g_id)
